# Graph Report - D:\Codexgeneratedwebsite  (2026-04-19)

## Corpus Check
- 10 files · ~1,437,957 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 36 nodes · 35 edges · 10 communities detected
- Extraction: 77% EXTRACTED · 23% INFERRED · 0% AMBIGUOUS · INFERRED: 8 edges (avg confidence: 0.8)
...
- `getSiteData()` --calls--> `getMetaDescription()`  [INFERRED]
  D:\Codexgeneratedwebsite\src\lib\content.ts → D:\Codexgeneratedwebsite\src\lib\seo.ts
- `getSiteData()` --calls--> `getMetaKeywords()`  [INFERRED]
  D:\Codexgeneratedwebsite\src\lib\content.ts → D:\Codexgeneratedwebsite\src\lib\seo.ts
...
Cohesion: 0.38
Nodes (5): getSiteData(), GET(), getMetaDescription(), getMetaKeywords(), getMetaTitle()
...
## Knowledge Gaps
- **Thin community `Community 6`** (2 nodes): `resolveProductColorCollectionItem()`, `resolveProductImage()`
...
- **Why does `getSiteData()` connect `Community 1` to `Community 0`?**
  _High betweenness centrality (0.324) - this node is a cross-community bridge._
- **Why does `toAbsoluteUrl()` connect `Community 2` to `Community 1`?**
  _High betweenness centrality (0.085) - this node is a cross-community bridge._
- **Why does `getCanonicalUrl()` connect `Community 2` to `Community 1`?**
  _High betweenness centrality (0.085) - this node is a cross-community bridge._
- **Are the 4 inferred relationships involving `getSiteData()` (e.g. with `getMetaTitle()` and `getMetaDescription()`) actually correct?**
  _`getSiteData()` has 4 INFERRED edges - model-reasoned connections that need verification._
